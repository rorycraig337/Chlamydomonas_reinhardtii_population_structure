...
while (my $iso = <IN1>) {
	chomp $iso;
	my @iso_info = split(/\t/, $iso);
	print OUT "$iso_info[0],$iso_info[0],0,0,0\n";
	$iso_count++;
...
open (OUT1, ">A_$out") or die;
open (OUT2, ">B_$out") or die;
	...
	if ( ($count < 10) and ($count != 1) ) { #these cols have the isolate number as [ *], so remove first part element of @cols
		...
		foreach my $gen_distance (@cols) {
			...
			if ( ($clade1 eq "A") and ($clade2 eq "A") ) {
				$type = "0";
				print OUT1 "$iso1,$iso2,$gen_distance,$geo_distance,$type\n";
			}
				...
				$type = "0";
				print OUT2 "$iso1,$iso2,$gen_distance,$geo_distance,$type\n";
			}
			else {
